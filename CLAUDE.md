...
wells, with a third stiffness level available to soften the backbone;
`duffing.py` is behind its numbers and its `figures/duffing-*` images.
`LOTKA.md` is the seventh prototype and the first whose states are
